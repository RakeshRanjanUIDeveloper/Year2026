...
const categoryConfig = {
  language:  { color: 'from-purple-500 to-indigo-500',  bg: 'bg-purple-500',  text: 'text-purple-400',  border: 'border-purple-500/30' },
  framework: { color: 'from-cyan-500 to-blue-500',      bg: 'bg-cyan-500',    text: 'text-cyan-400',    border: 'border-cyan-500/30' },
  state:     { color: 'from-pink-500 to-rose-500',      bg: 'bg-pink-500',    text: 'text-pink-400',    border: 'border-pink-500/30' },
  styling:   { color: 'from-violet-500 to-purple-500',  bg: 'bg-violet-500',  text: 'text-violet-400',  border: 'border-violet-500/30' },
  testing:   { color: 'from-green-500 to-emerald-500',  bg: 'bg-green-500',   text: 'text-green-400',   border: 'border-green-500/30' },
  platform:  { color: 'from-amber-500 to-orange-500',   bg: 'bg-amber-500',   text: 'text-amber-400',   border: 'border-amber-500/30' },
  tools:     { color: 'from-teal-500 to-cyan-500',      bg: 'bg-teal-500',    text: 'text-teal-400',    border: 'border-teal-500/30' },
}
...
function TechStack() {
  ...
  return (
    <section className="py-32 px-6 max-w-6xl mx-auto relative overflow-hidden">
      ...
      <div className="absolute top-20 right-0 w-96 h-96 rounded-full blur-[120px] pointer-events-none" style={{ background: 'var(--blob-1)' }} />
      <div className="absolute bottom-20 left-0 w-80 h-80 rounded-full blur-[100px] pointer-events-none" style={{ background: 'var(--blob-3)' }} />
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} className="mb-16">
        <h2 className="text-5xl md:text-6xl font-bold leading-tight mb-4" style={{ color: 'var(--text-primary)' }}>
          Tools of
          <span className="block text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-400">my trade.</span>
        </h2>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="mb-14">
        <p className="text-xs tracking-widest uppercase font-mono mb-5" style={{ color: 'var(--text-muted)' }}>Core expertise</p>
          ...
          {topSkills.map((tech, i) => {
            ...
            return (
              <motion.div
                ...
                className={'group relative overflow-hidden rounded-2xl border p-5 cursor-default transition-all duration-300 hover:bg-white/10 ' + config.border}
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <span className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>{tech.level}%</span>
                  </div>
                </div>
              </motion.div>
            )
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }} className="mb-8">
        <p className="text-xs tracking-widest uppercase font-mono mb-5" style={{ color: 'var(--text-muted)' }}>Browse by category</p>
        <div className="flex flex-wrap gap-2">
          {tabs.map((tab) => {
            const count = tab.key === 'all' ? techStack.length : techStack.filter((t) => t.category === tab.key).length
            return (
              <button
                key={tab.key}
                ...
                className={'px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 flex items-center gap-2 ' +
                  (activeTab === tab.key
                    ...
                    : 'border border-white/10 hover:text-white hover:bg-white/10')}
                style={activeTab !== tab.key ? { color: 'var(--text-secondary)', backgroundColor: 'var(--card-bg)' } : {}}
              >
                {tab.label}
          ...
          {filtered.map((tech, i) => {
            ...
            return (
              <motion.div
                ...
                className={'group relative cursor-default rounded-2xl border-2 p-4 transition-all duration-200 hover:shadow-lg ' + config.border}
style={{ backgroundColor: 'var(--card-bg)' }}
                style={{ backgroundColor: 'var(--card-bg)' }}
              >
      ...
      {/* Bottom Stats */}
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            whileHover={{ y: -4 }}
            className="border border-white/10 rounded-2xl p-5 text-center hover:bg-white/10 hover:border-white/20 transition-all duration-300"
            style={{ backgroundColor: 'var(--card-bg)' }}
          >
            <div className={'text-3xl font-bold mb-1 text-transparent bg-clip-text bg-gradient-to-r ' + stat.color}>
              {stat.value}
            </div>
            <div className="text-xs tracking-wide uppercase" style={{ color: 'var(--text-secondary)' }}>{stat.label}</div>
          </motion.div>
        ))}
      </motion.div>
